...
export default function Piano({ onKeyPress, feedbackNote, correct }) {
  const getWhiteKeyClasses = (note) => {
    ...
    return 'bg-gray-50 dark:bg-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-300 active:bg-gray-200';
  };
  ...
  return (
    <div className="relative select-none" style={{ width: 'min(490px, 90vw)', height: 'min(180px, 33vw)' }}>
      ...
      <div className="flex h-full gap-[2px]">
        {WHITE_KEYS.map((note) => (
          <button
            ...
            className={`relative flex-1 border border-gray-200 dark:border-zinc-500 cursor-pointer transition-colors duration-100 flex items-end justify-center pb-3 first:rounded-l-xl last:rounded-r-xl ${getWhiteKeyClasses(note)}`}
          >
      ...
      {BLACK_KEYS.map(({ note, left }) => (
        <div
          key={note}
          className={`absolute top-0 ${left} w-8 h-[60%] bg-gray-800 dark:bg-zinc-900 rounded-md border border-gray-700 dark:border-zinc-700 pointer-events-none z-10`}
        />
      ))}
    </div>
  );
